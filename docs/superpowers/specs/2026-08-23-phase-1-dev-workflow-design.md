...
Verified against current packwiz docs (`packwiz init` flags, installer, export), Forge files (latest 1.20.1 build as of 2026-07-21 used only as the **init seed**), Pelican CurseForge Generic egg (`PROJECT_ID`, `VERSION_ID`, `API_KEY`), and `Kira-NT/mc-publish@v3`.
...
**Phase 1 is done when** tag `v0.0.1` produces a GitHub Release with both empty-pack artifacts. CurseForge/Modrinth skip-with-log is success until project IDs and tokens exist. Pelican is documented, not a Phase 1 gate.
...
- One pack + packwiz `side` (`client` / `server` / `both`). Pelican/ops/JVM overlay in `server/`, never indexed.
- Local Prism: `packwiz serve` → `http://localhost:8080/pack.toml`. Pelican does **not** use GitHub raw or packwiz-installer.
- Pelican: CurseForge Generic egg. Tracks the last **published CurseForge file**, not live git.
...
The-End-Game/
  ...
  server/                       # Pelican overlay — never in packwiz index
    README.md
...
- Egg: Pelican [CurseForge Generic](https://raw.githubusercontent.com/pelican-eggs/minecraft/refs/heads/main/java/curseforge/egg-curse-forge-generic.json)
...
- `PROJECT_ID`: CurseForge modpack project ID (set in Pelican, not git)
- `VERSION_ID`: `latest` on the test server (follows newest CF file)
- `API_KEY`: CurseForge console key in Pelican, never in git
...
| `test-server` | Auto when testing Pelican | Reinstall/restart, wait for boot, pull console, flag crash/error patterns and the egg’s “not a server pack” warning. Skill text includes Pelican API call **placeholders** until an MCP connection exists (intentional, not a missing spec). If no CF file exists, report blocked-on-publish |
...
### 3. Server test (Pelican, CurseForge Generic egg)
...
Phase 1 does not require a live Pelican install.
...
- Pelican: no CF file yet → blocked-on-publish, no git-URL fallback. Bad `API_KEY` / `PROJECT_ID` → egg ERROR in console. Key never in git. Egg “not a server pack” warning is flagged by `test-server`. Boot `Exception` / `Error` → pack not marked good.
...
Secrets and Pelican `API_KEY` never appear in logs, AGENTS.md, or the spec beyond **names**.
