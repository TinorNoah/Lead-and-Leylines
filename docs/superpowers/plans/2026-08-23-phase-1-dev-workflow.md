...
**Architecture:** `pack/` is the packwiz root. Repo-root docs, CI, and `.agents/` are never exported. `pack/pack.toml` is the only durable Minecraft/loader/version record. Pelican uses the CurseForge Generic egg (documented, not a Phase 1 gate). Prism uses `packwiz serve` + installer bootstrap.
...
| `server/README.md` | Pelican CurseForge Generic egg vars; no version literals |
...
| `.agents/skills/test-server/SKILL.md` | Pelican test with API placeholders |
...
git commit -m "docs: describe Pelican CurseForge Generic egg overlay"
```
...
**Spec coverage:** Layout, packwiz init seed, `.gitignore`/`.gitattributes`, `pack.toml` SoT, version reader, release workflow, server README, AGENTS/CLAUDE/CONTRIBUTING/README, three skills, empty export proof, push/tag gated on approval. Pelican is documented, not a Phase 1 gate.

**Placeholders:** Pelican API `# TODO` lines are required by the spec (skill placeholders until MCP exists), not incomplete plan steps.
