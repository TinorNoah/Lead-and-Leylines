# Installed mods

Browse the mods in the pack by what they add. Compat addons sit in the same table as the mod they extend.

Why a mod was kept or held is in [`docs/mods/`](../mods/manifest.md). This folder is only the current install.

Edit [`catalog.toml`](catalog.toml), then run `python3 scripts/installed_catalog.py` to regenerate these pages and [`catalog.json`](catalog.json). Who updates what: [`MAINTENANCE.md`](MAINTENANCE.md).

The searchable browser UI lives in [`site/`](../../site/README.md) (Docker / Dokploy).

| Category | Mods | What you'll find |
|---|---:|---|
| [Weapons](weapons.md) | 44 | Guns and the addons that change those guns. |
| [Combat](combat.md) | 15 | How the player fights in melee and moves. |
| [Bosses](bosses.md) | 13 | Boss mods and the structure addons that rebuild their arenas. |
| [Mobs](mobs.md) | 20 | Creatures, wildlife, and the libraries those mods need. |
| [Structures](structures.md) | 27 | Dungeons, villages, and the tools that space or label them. |
| [Biomes](biomes.md) | 20 | Overworld and cave biome sets. |
| [The Nether](nether.md) | 13 | Nether biomes, structures, and mobs. |
| [The End](end.md) | 3 | End biomes and the End expansion. |
| [Magic](magic.md) | 41 | Spell mods and their addons. |
| [Food](food.md) | 18 | Kitchens and cooking. |
| [Farming](farming.md) | 4 | Pots and area harvest. |
| [Seasons](seasons.md) | 5 | The season clock. |
| [Storage](storage.md) | 24 | Backpacks, chests, and trash. |
| [Applied Energistics](applied-energistics.md) | 26 | ME storage and autocrafting. |
| [Refined Storage](refined-storage.md) | 5 | Disks, grids, and the addons that extend them. |
| [Mekanism](mekanism.md) | 10 | Machines, factories, gases, and tools. |
| [Create](create.md) | 10 | Contraptions and kinetics. |
| [Transport](transport.md) | 12 | Vehicles, fast travel, and item pipes. |
| [Resources](resources.md) | 6 | Ores, unification, and chemistry. |
| [Building](building.md) | 6 | Building tools and decoration. |
| [Exploration](exploration.md) | 4 | Maps and dimensions. |
| [FTB](ftb.md) | 10 | Claims, teams, quests, and the shared FTB library. |
| [Gameplay](gameplay.md) | 14 | Small world and inventory changes. |
| [Recipes and overlays](info.md) | 22 | JEI, Jade, and tooltip frames. |
| [Rendering](rendering.md) | 21 | Shaders, entity models, and client render optimizers. |
| [Client](client.md) | 18 | Client-only comfort. |
| [Performance](performance.md) | 19 | Tick, memory, and chunk-saving work that runs on the server and in singleplayer. |
| [Stability](stability.md) | 9 | Crash isolation, leak patches, and network fixes. |
| [Libraries](libraries.md) | 12 | Shared libraries used by more than one mod family. |

451 entries, each listed once.
