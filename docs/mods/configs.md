...
| Streams Reflowing | Pack override | Pin `2.13.7` (`OZN2NMml`). Ship `pack/config/streamsreflowing-common.toml` `terrainAccuracyLevel = "LOW"` so Tectonic watersheds do not stall chunk gen (author default MEDIUM). New chunks only, after restart. Existing MEDIUM land may not line up at the edge. Do not add Ksyxis or C2ME. |
| Epic Structures: Dungeons | Version pin | `1.0` (`fMMms6dF`), the 1.20-native jar. 1.1+ store 1.21 `components` on item-frame loot even when Modrinth tags 1.20. Villages / witch huts / jungle temples stay on current files (no `components` NBT). |
...
| Distant Horizons | Pack override | Client only. `pack/config/DistantHorizons.toml`: `rendererMode = "DISABLED"`, `enableDistantGeneration = false`. Floor quality if the player turns it on in Options → DH. Auto-updater off. Do not put DH on the dedicated server. Java 17 uses `-XX:+UseZGC` (DH's G1 warning; not a second mod). Source of truth is `pack/user_jvm_args.txt` (exported into `.minecraft`). `python scripts/update_prism.py` copies those flags into Prism `instance.cfg`. Mrpack/CurseForge cannot auto-apply launcher JVM args — ATLauncher/store installs paste the same flags. Do not add `-XX:+ZGenerational` (Java 21+). |
