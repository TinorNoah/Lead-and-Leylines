...
4. Terralith + RU + BWG is three surface biome injectors. Terralith biomes become rarer and smaller. Accepted.
...
| TerraBlender | [modrinth.com/mod/terrablender](https://modrinth.com/mod/terrablender) `3.0.1.10` (`zGconCHG`) | both | Shared by RU and BWG |
...
| Tectonic | [modrinth.com/mod/tectonic](https://modrinth.com/mod/tectonic) `3.0.17` (`KLmvRxwh`) | both | Big terrain. Spacing tweak in [configs.md](configs.md) |
...
| Structurify | [modrinth.com/mod/structurify](https://modrinth.com/mod/structurify) `2.0.37` (`FMlB4OQ1`) | both | Structure Control (spacing / enable) |
...
| Sparse Structures | [modrinth.com/mod/sparsestructures](https://modrinth.com/mod/sparsestructures) `3.0` (`oqEnX6AN`) | both | Global structure spacing |
| StructureOverlapless | [curseforge.com/.../structureoverlapless](https://www.curseforge.com/minecraft/mc-mods/structureoverlapless) `1.2.0` (file `7722757`) | both | Skip overlapping structures |
...
| WDA Sparse compat packs | `bI23vAlR` / `BzLuY8LF` | both | Keep WDA a bit rare next to Sparse Structures |
| Create: Sky Village | [modrinth.com/mod/create-sky-village](https://modrinth.com/mod/create-sky-village) `0.0.38` (`630wzTP1`) | both | Create is already in |
...
| Tectonic | After first launch: Continents Scale `0.11` (default `0.13`), Ocean Offset `-0.92` (default `-0.8`). Size stays large; more ocean between land. Do not raise Ocean Offset toward `0`. |
...
| Chunky | Pregen writes the world. Use on purpose; do not leave a huge radius running unattended. |
| Terrain Slabs | Beta. |
