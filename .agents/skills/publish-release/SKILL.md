...
description: Publish a pack version from a git tag and changelog (GitHub Release, CurseForge, Modrinth, dedicated test server, then local Prism). Use only when the user explicitly types /publish-release.
disable-model-invocation: true
---

# Publish a release

The operator machine tags and creates the GitHub Release, then updates the live instance and local Prism. GitHub Actions publishes CurseForge/Modrinth from every `v*` tag (default alpha).
...
5. From the repo root run `python scripts/release.py vX.Y.Z --channel alpha` (`release` only if the user asked for a public stable). That tags, creates the GitHub Release (client zip, mrpack, and server-mods zip), then **always** `python scripts/deploy_server.py --from-local` unless `--skip-server`, then **always** `python scripts/update_prism.py` unless `--skip-prism`. The tag push starts `.github/workflows/publish-stores.yml`, which uploads CurseForge/Modrinth as alpha when the GitHub Release is a prerelease. `--dry-run`, `--skip-server`, `--skip-prism`, and `--upload-stores` exist when the user asks. Push only when the user asked to release or push.
6. Point the user at the GitHub Release URL and the Publish stores workflow run. Store tokens live as GitHub Actions secrets. `MODRINTH_PROJECT_ID` must be the 8-character dashboard id, not the slug. `GH_TOKEN` is required on the operator machine.
