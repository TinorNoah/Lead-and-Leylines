...
- Packwiz root is `pack/`; docs (including `docs/mods/` decision logs) and `.agents/` stay at the repo root and are not exported. Shaders are a pack feature (Oculus + Embeddium); Colorwheel is the Create + Oculus path. Do not add a separate dynamic-lights mod while Oculus is in.
- Branching is GitHub Flow (`main`, feature branches, PRs); pack version `X.Y.Z` matches git tag `vX.Y.Z`. `python scripts/release.py vX.Y.Z --changelog <file>` creates the GitHub Release from that changelog (no tag Actions job).
