...
Never hardcode those values. Missing CurseForge or Modrinth values skip that store with an explicit log. Missing `GH_TOKEN` skips the GitHub Release but still updates the test server. There is no tag GitHub Actions publish job.
