...
- Record mod decisions in `docs/mods/` (considered, chosen, held, or dropped; rationale; packwiz `side`); document only configs that need pack notes. `docs/mods/deferred.md` is the not-yet-ported 1.20.1 list plus researched skip/hold reasons, not an install queue.
...
- When porting combat and storage from the old pack, use Epic Fight + ParCool (not Better Combat) and keep both Applied Energistics 2 and Refined Storage.
...
- This is a long-term Minecraft NeoForge modpack managed with packwiz; never commit jars. `main` is the 1.21.1 NeoForge pack (official Sodium / Iris / Lithium, not Embeddium / Oculus / Radium). The 1.20.1 Forge pack is archived on `forge-1.20.1` (tag `archive/forge-1.20.1`); do not copy that content onto `main`.
