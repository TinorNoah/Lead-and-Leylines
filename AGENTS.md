...
- From repo root: `python scripts/release.py vX.Y.Z --changelog notes.md` — GitHub Release; then dedicated test server; then local Prism (`scripts/update_prism.py`). Tag `v*` starts GitHub Actions for CurseForge + Modrinth (default alpha). Changelog is the public notes only.
...
- `.github/workflows/publish-stores.yml` — CurseForge + Modrinth from every `v*` tag (default alpha)
...
Never hardcode those values. CurseForge and Modrinth publish from GitHub Actions secrets on every `v*` tag (default alpha). Local `.env` store keys are optional (`--upload-stores`). Missing `GH_TOKEN` skips the GitHub Release but still updates the test server and local Prism.
...
- Packwiz root is `pack/`; docs (including `docs/mods/` decision logs) and `.agents/` stay at the repo root and are not exported. Shaders are a pack feature (Oculus + Embeddium); Colorwheel is the Create + Oculus path. Embeddium is the renderer — do not add OptiFine, Nvidium, VulkanMod, or a separate dynamic-lights mod while Oculus is in. Distant Horizons is client-only and off by default (`rendererMode` disabled, no distant generation); do not put DH on the dedicated server.
- Branching is GitHub Flow (`main`, feature branches, PRs); pack version `X.Y.Z` matches git tag `vX.Y.Z`. `python scripts/release.py vX.Y.Z` creates the GitHub Release from `CHANGELOG.md`. `.github/workflows/publish-stores.yml` uploads CurseForge/Modrinth from that tag.
